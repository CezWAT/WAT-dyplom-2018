    ...
    def open(self):
        ...
        if self.s.isOpen():
            ...
            if s  == 'MODEL: B3606':
                ...
                print 'Couldnt read the right model out of the serial port, got "%s", expected to see "MODEL: B3606"' % s
        else:
            return False
...
def auto_calibration():
    psu = B3606(sys.argv[2])
    if not psu.open():
        print 'Failed to open serial port to B3606 on serial %s' % sys.argv[2]
        return
    ...
    for step in xrange(NUM_STEPS):
        ...
        if vout == None:
            ...
            break
        if vout < 0.1:
            print 'Vout is too low (%s), something broke, try to reset the B3606 to defaults with RESTORE command first' % vout
            valid = False
            break
        if vin - vout < 0.5:
    ...
    if adc_b_tmp < 0:
        ...
        print 'Expected ADC_B to be negative... for some reason it\'s not'
        adc_b_tmp = 0
...
def usage():
    print 'Usage: %s -a <b3606 serial> <multimeter serial> <multimeter model>' % sys.argv[0]
    print
    print 'or:'
    print
    print '%s -m <b3606 serial>' % sys.argv[0]
