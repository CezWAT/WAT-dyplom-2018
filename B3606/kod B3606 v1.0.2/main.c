...
void write_milli(const char *prefix, uint16_t val) //milli/amps/volts are passed here
{
	uart_write_str(prefix);
...
void write_uint(const char *prefix, uint16_t val) //integers only
{
	uart_write_str(prefix);
	uart_write_uint(val);
	uart_write_str("\r\n");
...
{
	if (s[1] != 0) {
		write_str("OUTPUT takes either 0 for OFF or 1 for ON, received: \"", s);
		uart_write_str("\"\r\n");
		return;
	}
...
void set_autocommit(uint8_t *s)
{
	if (strcmp(s, "1") == 0 || strcmp(s, "YES") == 0) {
		...
		write_str("UNKNOWN AUTOCOMMIT ARG: ", s);
		uart_write_str("\r\n");
	}
}
...
void process_input()
{
	...
	if (strcmp(uart_read_buf, "MODEL") == 0) {
		...
		write_milli("VMIN: ", CAP_VMIN);
		write_milli("VMAX: ", CAP_VMAX);
		write_milli("VSTEP: ", CAP_VSTEP);
		write_milli("CMIN: ", CAP_CMIN);
		write_milli("CMAX: ", CAP_CMAX);
		write_milli("CSTEP: ", CAP_CSTEP);
	} else if (strcmp(uart_read_buf, "CONFIG") == 0) {
		uart_write_str("CONFIG:\r\n");
		write_onoff("OUTPUT: ", cfg_system.output);
		write_milli("VSET: ", cfg_output.vset);
		write_milli("CSET: ", cfg_output.cset);
		write_milli("VSHUTDOWN: ", cfg_output.vshutdown);
		write_milli("CSHUTDOWN: ", cfg_output.cshutdown);
	} else if (strcmp(uart_read_buf, "STATUS") == 0) {
		uart_write_str("STATUS:\r\n");
		write_onoff("OUTPUT: ", cfg_system.output);
		write_milli("VIN: ", state.vin);
		write_milli("VOUT: ", state.vout);
		write_milli("COUT: ", state.cout);
...
inline void pinout_init()
{
	...
	PA_ODR = 0;							//Port A data output latch register 
	PA_DDR = (1<<1) | (1<<2);			//Port A data direction register 
	PA_CR1 = (1<<1) | (1<<2) | (1<<3);	//Port A control register 1
	PA_CR2 = (1<<1) | (1<<2) | (1<<3);	//Port A control register 2
...
void read_state(void)
{
	...
	tmp = (PC_IDR >> 3) & 1; //input pin value register bit shift 3 to the right with bitwise and by 1
	if (state.pc3 != tmp) {
...
int main()
{
	...
	uart_write_str(MODEL " starting: Version " FW_VERSION "\r\n");
